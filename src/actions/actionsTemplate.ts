...
export default class actionsTemplate {
  ...
  LIST_SUCCESS: string;
  LIST_FAIL: string;
  LIST_MANY_REQUEST: string;
  LIST_MANY_SUCCESS: string;
  LIST_MANY_FAIL: string;
  ...
  constructor(constants: constantsTemplate, api: string) {
    ...
    this.LIST_MANY_REQUEST = constants.constants().LIST_MANY_REQUEST;
    this.LIST_MANY_SUCCESS = constants.constants().LIST_MANY_SUCCESS;
    this.LIST_MANY_FAIL = constants.constants().LIST_MANY_FAIL;
  ...
  listData = (id: string | undefined) => async (dispatch: Dispatch) => {
    dispatch({ type: this.LIST_MANY_REQUEST });
    try {
      const { data } = await Axios.get(`${this.URL}/${this.api}/${id}`);
      console.log("Actions data", data)
      dispatch({ type: this.LIST_MANY_SUCCESS, payload: data });
    } catch (err) {
      dispatch({ type: this.LIST_MANY_FAIL });
    }
  };
